...
def preprocess(datadir, outdir):
    ...
    dists.build(os.path.join(datadir, 'train_all.txt'), continous_features)

    dicts = CategoryDictGenerator(len(categorial_features))
    dicts.build(
        os.path.join(datadir, 'train_all.txt'), categorial_features, cutoff=200)
    ...
    # for validation.
    with open(os.path.join(outdir, 'train_all.txt'), 'w') as out_train:
        with open(os.path.join(outdir, 'valid_all.txt'), 'w') as out_valid:
            with open(os.path.join(datadir, 'train_all.txt'), 'r') as f:
                for line in f:
                    features = line.rstrip('\n').split('\t')
